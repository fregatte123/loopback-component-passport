...
PassportConfigurator.prototype.setupModels = function(options) {
  ...
  this.accessTokenModel =  loopback.getModelByType(this.app.models.AccessToken);
...
PassportConfigurator.prototype.configureProvider = function(name, options) {
      ...
      strategy = new AuthStrategy(_.defaults({
        ...
        function(req, accessToken, refreshToken, profile, done) {
          if (link) {
            ...
            //此处记录的是系统本身登陆的accesstoken  用来实现用户已经登陆了  想绑定微信或者其他第三方账户的功能
            var accessTokenId = req.accessToken?req.accessToken.id:undefined;
            //
            if(accessTokenId){
              self.accessTokenModel.findById(accessTokenId,function(err,accessToken){
                if (err || !accessToken) {
                  return done(err, accessToken);
                }
                self.userIdentityModel.login(provider, authScheme, profile,
                {accessToken: accessToken, refreshToken: refreshToken},
                _.defaults({userId:accessToken.userId},options), loginCallback(req, done));
              }); 
            } else {
              self.userIdentityModel.login(provider, authScheme, profile,
              {accessToken: accessToken, refreshToken: refreshToken},
  ...
  if (authType === 'local') {
    ...
    self.app.get(authPath, passport.authenticate(name, _.defaults({
      scope: scope,
      session: session,
    }, options.authOptions,options.authCallback)),flashmsg);
  }
  function flashmsg(req,res){
    res.send(req.flash());
  }
  ...
  if (link) {
    ...
    self.app[callbackHTTPMethod](callbackPath, customCallback,flashmsg);
  }
